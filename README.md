...
This is the full agent loop: scan → evaluate (OPA) → **reason** (Bedrock) → surface PR. Bedrock analyses the violations, explains the likely cause, generates specific file changes, and assesses risk. The PR includes the reasoning and the fix.
...
Dry-run mode skips the Bedrock reasoning step and uses a hardcoded response. Use this to **test the pipeline** (OPA evaluation, PR creation, OTel tracing) without AWS credentials. The PR is still real, but the reasoning is static — not generated by the agent.

> ⚠️ **Dry-run is for testing the pipeline, not for production.** Without Bedrock, the agent cannot reason over drift — it becomes a scanner, not an agent.
